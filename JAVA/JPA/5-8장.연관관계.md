...
### 다대일 단방향 [N:1]
- 회원 엔티티(N), 팀 엔티티(1)
- ManyToOne 어노테이션을 붙인다.

### 다대일 양방향 [N:1, 1:N]
- @ManyToOne, @OneToMany 를 각각 붙여준다.

## 일대다
엔티티를 하나 이상 참조할 수 있으므로 자바 컬렉션인 Collection, List, Set, Map 중에 하나를 사용해야 한다.
### 일대다 단방향
* Team, Member를 예로 들 때, Team.members로 회원 테이블의 TEAM_ID 외래 키를 관리.
* 반대쪽 테이블에 있는 외래 키를 관리
* @JoinColumn(name="TEAM_ID") //member테이블의 TEAM_ID 어노테이션 명시
* 일대다 단방향 매핑보다 다대일 양방향 매핑 사용하자. (일대다 양방향은 없음)
### 일대다 양방향(=일대다 단방향 매핑 + 다대일 단방향 매핑 읽기 전용 추가)
* 다대일 반대쪽은 `insertable=false`, `updatable=false`로 설정하여 읽기만 가능하게 구성.

## 일대일
1. 주 테이블에 외래 키 갖는 경우
2. 대상 테이블에 외래 키 갖는 경우
